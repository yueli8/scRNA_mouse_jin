...
# 2. 提取 CD4+ T 细胞亚群 (activated cd4, cd4 Th2, naive cd4, treg)
# 3. 拆分为 G1~G4 四个组
...
cd4 <- subset(
  hms_cluster_id, 
  idents = c("activated cd4", "cd4 Th2", "naive cd4", "treg")
)

# 可视化 subset 后的数据
DimPlot(cd4, reduction = "umap", label = TRUE, pt.size = 0.5)
DimPlot(cd4, reduction = "umap", label = FALSE, pt.size = 0.5)

# 查看细胞类型分布
print("CD4 亚群细胞类型分布：")
print(table(Idents(cd4)))
...
# 检查样本是否存在
all_samples <- unique(cd4@meta.data$orig.ident)
print("当前数据中包含的样本：")
print(all_samples)

# 拆分
cd4_G1 <- subset(cd4, subset = orig.ident %in% g1_samples)
cd4_G2 <- subset(cd4, subset = orig.ident %in% g2_samples)
cd4_G3 <- subset(cd4, subset = orig.ident %in% g3_samples)
cd4_G4 <- subset(cd4, subset = orig.ident %in% g4_samples)

# 打印各组细胞数
print(paste("G1 细胞数:", ncol(cd4_G1)))
print(paste("G2 细胞数:", ncol(cd4_G2)))
print(paste("G3 细胞数:", ncol(cd4_G3)))
print(paste("G4 细胞数:", ncol(cd4_G4)))

# 保存拆分后的 Seurat 对象（可选）
saveRDS(cd4_G1, "cd4_G1_subset.rds")
saveRDS(cd4_G2, "cd4_G2_subset.rds")
saveRDS(cd4_G3, "cd4_G3_subset.rds")
saveRDS(cd4_G4, "cd4_G4_subset.rds")
...
run_monocle_analysis <- function(seurat_obj, group_name) {
  ...
  # --- 4.7 排序细胞（拟时序） ---
  # CD4 起始：优先使用 activated cd4 或 naive cd4
  root_cells <- colnames(cds)[colData(cds)$celltype == "activated cd4"]
  ...
  # --- 4.8 保存 Monocle3 对象 ---
  saveRDS(cds, paste0(group_name, "_cd4_monocle3_final.rds"))
  
  # --- 4.9 可视化（拟时序 + 细胞类型） ---
  p1 <- plot_cells(cds, color_cells_by = "pseudotime",
                   label_cell_groups = FALSE, label_leaves = FALSE,
                   label_branch_points = FALSE) +
    ggtitle(paste(group_name, "CD4 Pseudotime")) + theme_minimal() +
  ...
  p2 <- plot_cells(cds, color_cells_by = "celltype",
    ...
    ggtitle(paste(group_name, "CD4 Cell Types in Trajectory")) + theme_minimal()
  print(p2)
  
  ggsave(paste0(group_name, "_cd4_pseudotime.pdf"), p1, width = 10, height = 8)
  ggsave(paste0(group_name, "_cd4_celltypes.pdf"), p2, width = 10, height = 8)
  
  return(cds)
}

# 分别对四组运行 Monocle3 分析
cds_G1 <- run_monocle_analysis(cd4_G1, "G1")
cds_G2 <- run_monocle_analysis(cd4_G2, "G2")
cds_G3 <- run_monocle_analysis(cd4_G3, "G3")
cds_G4 <- run_monocle_analysis(cd4_G4, "G4")

# =====================================================================
# 5. 对 G1~G4 分别绘制 40 基因拟时序表达图（显示 + 保存）
# =====================================================================

# 40 个基因列表（小鼠基因，CD4 T 细胞相关）
target_genes <- c(
  # --- CD4 T 细胞身份与受体 ---
  "Cd3e", "Cd3d", "Cd4", "Trac", "Cd28", "Cd27", "Icos", "Cd40lg",
  
  # --- Th1 相关 ---
  "Ifng", "Tbx21", "Stat1", "Cxcr3", "Il12rb2",
  
  # --- Th2 相关 ---
  "Gata3", "Il4", "Il5", "Il13", "Ccr4", "Stat6",
  
  # --- Th17 相关 ---
  "Rorc", "Il17a", "Il17f", "Il23r", "Ccr6", "Stat3",
  
  # --- Treg 相关 ---
  "Foxp3", "Il2ra", "Ctla4", "Ikzf2", "Tnfrsf18", "Tnfrsf4", "Nt5e",
  
  # --- Tfh 相关 ---
  "Bcl6", "Cxcr5", "Il21", "Pdcd1", "Sh2d1a",
  
  # --- 激活与记忆 ---
  "Cd69", "Sell", "Tcf7", "Lef1", "Ccr7", "Il7r",
  
  # --- 增殖与耗竭 ---
  "Mki67", "Top2a", "Stmn1", "Havcr2", "Lag3", "Tigit"
)
...
for (group_name in names(group_list)) {
  ...
  p_genes <- ggplot(plot_data, aes(x = pseudotime, y = expression)) +
    ...
    labs(
      title = paste0(group_name, " Group: ", length(valid_genes),
                     " Genes Expression Along Pseudotime (CD4 + T Cells)"),
  ...
  output_file <- paste0(group_name, "_40_Genes_Pseudotime_CD4T.pdf")
  ...
  box_file <- paste0(group_name, "_cd4_Pseudotime_Boxplot.pdf")
  ggsave(box_file, p_box, width = 10, height = 8)
  cat(paste(group_name, "箱线图已保存为:", box_file, "\n"))
}

cat("\n========================================\n")
cat("全部流程（G1~G4 CD4+ T 细胞）分析完成！\n")
cat("========================================\n")